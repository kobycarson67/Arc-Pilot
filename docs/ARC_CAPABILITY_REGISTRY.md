...
Verified against the local `sidebar-gesture-repair-2` candidate, schema v7. The commit and tree for this candidate are recorded in its implementation report. Update this registry whenever a later milestone changes a capability.
...
| Students | Implemented | `ArcCurrentStudentDirectory` projection; sidebar Students directory | Read only / fixture enrollments | Workflow and Sidebar Gesture Repair 2 tests | Renamed from Students / Roster; destination, records, roster, and enrollment authority are unchanged. |
...
| Open Shop | Implemented | `gaps`, `viewStudentCompetency`; class and Student Summary exact-competency actions | Derived / fixture ratings | Pre-Titanium and workflow repair tests | Ranking uses existing assessed gaps; no new recommendation engine. |
...
| Project Library | Implemented | `state.projectBank`; sidebar Project Library and existing definition editor | State / fixture bank | Project Bank, workflow, and Sidebar Gesture Repair 2 tests | User-facing rename/grouping only; underlying Project Bank data and behavior remain unchanged. Four WT starters, no AWT starter content. |
| Checkpoints | Implemented | `ArcProjectCheckpoints` and controller; student Projects; Titanium status presentation for every authoritative state | Student assignment / fixture checkpoints | Checkpoint domain/integration and workflow repair tests | Progress evidence is separate from grade. |
| Project grading/rubrics | Implemented | `PROJECT_RUBRIC`, `projectScore`; Titanium student Projects/Grades surface | Student assignment rubric / fixture rubric | Static, project, and workflow repair tests | Existing instructor criteria, weights, ratings, and calculations are unchanged; rubric redesign remains future. |
...
| Class Forecast | Implemented | `ArcClassForecast`; selected-class Forecast/Dashboard pulse; project links stop at the exact assignment card | Derived, no forecast store / fixture inputs | Forecast domain/integration and workflow repair tests | Preparation facts, not predictions; ranking and Up Next semantics are unchanged. |
...
| Lesson Plans | Implemented | Master lesson bank and pacing; Lesson Plans | State / fixture base | Static, Teaching Tips, and Sidebar Gesture Repair 2 tests | User-facing rename only; underlying lesson records and behavior remain unchanged. Today's Focus class/date link is deferred. |
...
| Header live clock and countdown | Implemented | Single `ArcTimeLifecycle` controller; global header using `dayPlan`, current schedule context, active bell schedule, and overrides | Nonpersistent real device time in both modes | Workflow lifecycle and workflow repair tests | Shows active class or neutral Planning time remaining; no countdown without an authoritative active period. |
| Class cleanup indicator | Implemented | Header countdown and nonblocking ARC banner during the final 10 minutes of an active student class | Nonpersistent constant threshold; no classroom record writes | Workflow repair tests | Threshold is fixed at 10 minutes; future configuration is intentionally docked. |
...
| Simulation | Implemented | `ArcSimulationFoundation`, fixtures; Demo & Testing; ARC-native verified-entry feedback | Isolated simulation storage / canonical fictional fixtures | Simulation and workflow repair suites | Live snapshot verification and exact restoration remain authoritative. |
| Responsive sidebar touch gesture | Implemented | Unified mobile state authority with full collapsed-rail acquisition, rigid transformed drawer, progress/full-open backdrop, intercepted backdrop close, and existing drag/flick/toggle controls | UI state only; final open state uses existing shell/history authority | Workflow repair and Sidebar Gesture Repair 2 tests | Local verification complete; Samsung case-installed physical retest remains required. |
| Sidebar information architecture | Implemented | Classroom, Instruction, Activity Library, Shop, and ARC / Global groups in `ArcTitaniumShell` | Shell configuration only; no domain persistence | Titanium and Sidebar Gesture Repair 2 tests | Students, Lesson Plans, and Project Library retain existing routes. Gradebook, Today's Focus, Technical Assignments, Skill Challenges, Practice, Resources, Administration, and Help are inert FUTURE homes only. No global Showcase or Master Attendance reroute. |
