...
Verified against the local `arc-workflow-integration-1-repair-1` candidate, schema v7. The commit and tree for this candidate are recorded in its implementation report. Update this registry whenever a later milestone changes a capability.
...
| Responsive sidebar touch gesture | Implemented | Existing mobile sidebar authority plus bounded Pointer Events edge-open and drawer-close tracking | UI state only; final open state uses existing shell/history authority | Workflow repair gesture tests | Requires physical Samsung portrait/landscape acceptance. |
