...
(function(root,factory){
  ...
  function act(project,checkpointId,status,context){context=context||{};return C.transition(project,checkpointId,status,{timestamp:context.timestamp||'',date:todayFrom(context.timestamp,context.date),academicYear:context.academicYear||'',semester:context.semester||'',sectionId:context.sectionId||'',verifiedBy:context.verifiedBy||'',attendanceStatus:context.attendanceStatus||'',note:context.note,measurement:context.measurement,activityNote:context.activityNote||'',activityId:context.activityId||''});}
  ...
  function recordOpportunity(project,checkpointId,context){context=context||{};return C.addActivity(project,{id:context.activityId||'',checkpointId:checkpointId||'',type:'opportunity',date:todayFrom(context.timestamp,context.date),academicYear:context.academicYear||'',semester:context.semester||'',sectionId:context.sectionId||'',attendanceStatus:context.attendanceStatus||'',opportunityConfirmed:context.opportunityConfirmed===true,meaningfulProgress:false,note:context.note||''});}
