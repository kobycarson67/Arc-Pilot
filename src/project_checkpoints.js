...
(function(root,factory){
  ...
  var ACTIVITY_TYPES=['opportunity','progress','review','verification'];
  ...
  function validActivityType(type){return ACTIVITY_TYPES.indexOf(type)>=0;}
  ...
  function initializeProject(project,options){var copy=Object.assign({},project||{});if(hasCheckpointData(copy))copy.checkpoints=normalize(copy.checkpoints);else copy.checkpoints=create(options);copy.checkpointActivity=Array.isArray(copy.checkpointActivity)?copy.checkpointActivity.slice():[];return copy;}
  function readyForReview(project){var view=viewForProject(project);return !view.legacy&&view.progress.readyForReview;}
  function activityRecord(input){input=input||{};if(!validActivityType(input.type)||!input.date)return null;return {id:input.id||'',checkpointId:input.checkpointId||'',type:input.type,date:String(input.date),attendanceStatus:input.attendanceStatus||'',opportunityConfirmed:input.opportunityConfirmed===true,meaningfulProgress:input.meaningfulProgress===true,note:input.note||''};}
  function addActivity(project,input){var copy=initializeProject(project),record=activityRecord(input);if(!record)return copy;copy.checkpointActivity=copy.checkpointActivity.concat([record]);return copy;}
  function activitySummary(project){var records=(project&&Array.isArray(project.checkpointActivity)?project.checkpointActivity:[]).map(activityRecord).filter(Boolean),eligible=records.filter(function(r){return r.type==='opportunity'&&r.opportunityConfirmed;});var evidence=records.filter(function(r){return r.meaningfulProgress||r.type==='review'||r.type==='verification';}).map(function(r){return r.date;}).filter(Boolean).sort();return {eligibleOpportunities:eligible.length,opportunityConfirmed:eligible.length>0,lastEvidenceDate:evidence.length?evidence[evidence.length-1]:'',activityCount:records.length};}
  return {DEFINITIONS:DEFINITIONS,STATUSES:STATUSES,ACTIVITY_TYPES:ACTIVITY_TYPES,definitions:definitions,create:create,normalize:normalize,progress:progress,validStatus:validStatus,validActivityType:validActivityType,hasCheckpointData:hasCheckpointData,viewForProject:viewForProject,initializeProject:initializeProject,readyForReview:readyForReview,activityRecord:activityRecord,addActivity:addActivity,activitySummary:activitySummary};
}));
